...
var _ = Describe("DKIMKey controller", func() {
	...
	It("should allow changing DKIMKey spec when DNSEndpoint already exists with the same public key", func() {
		...
		Eventually(func() error {
			...
			dnsName, ok := endpoints[0].(map[string]interface{})["dnsName"].(string)
			if !ok {
				return fmt.Errorf("invalid dnsName")
			}
			if !strings.Contains(dnsName, newSelector) {
				return fmt.Errorf("selector not updated in dnsName: got %s, want contains %s", dnsName, newSelector)
			}
			return nil
		}).Should(Succeed())
