...
//+kubebuilder:webhook:path=/validate-externaldns-k8s-io-v1alpha1-dnsendpoint,mutating=false,failurePolicy=fail,sideEffects=None,groups=externaldns.k8s.io,resources=dnsendpoints,verbs=update;delete,versions=v1alpha1,name=vdnsendpoint.kb.io,admissionReviewVersions={v1}
...
func (v *dnsEndpointValidator) handleUpdate(req admission.Request) admission.Response {
	de := externaldns.DNSEndpoint()
	if err := v.dec.Decode(req, de); err != nil {
		return admission.Errored(http.StatusBadRequest, err)
	}
	owners := de.GetOwnerReferences()
	for _, owner := range owners {
		if owner.APIVersion == dkimmanagerv1.GroupVersion.String() && owner.Kind == dkimmanagerv1.DKIMKeyKind {
			return admission.Denied("directly updating DKIM record is not allowed")
		}
	}
	return admission.Allowed("")
}
