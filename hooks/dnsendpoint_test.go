...
var _ = Describe("DNSEndpoint webhook", func() {
	...
	It("should allow updating and deleting ordinary DNSEndpoints", func() {
		name := uuid.NewString()
			...
			"endpoints": []map[string]interface{}{
				{
					...
					"recordTTL":  3600,
					"recordType": "TXT",
					"targets":    []string{"new value"},
				},
			},
		}

		err = k8sClient.Update(ctx, de)
		Expect(err).NotTo(HaveOccurred())
	...
	It("should prevent modifying or deleting owned DNSEndpoints", func() {
		name := uuid.NewString()
		namespace := uuid.NewString()
		shouldCreateNamespace(ctx, namespace)
